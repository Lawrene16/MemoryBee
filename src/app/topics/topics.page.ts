...
export class TopicsPage implements OnInit {
  ...
  topicTitlesArray: any = ["Christology", "Covenants", "Doctrine of Man", "Eschatology", "Hermeneutics", "Soteriology", "Theology Proper"];
  ...
  getQuestions(jsonData, subject, topic) {
    ...
    // console.log(uniqueQuestionsArray);
  ...
  getDecks(jsonData, subject, topic) {
    ...
    return this.removeDuplicates(decksArray);
  }

  removeDuplicates(originalArray) {
    const filteredArr = originalArray.reduce((acc, current) => {
  ...
  testOrStudy(choice) {
    ...
    this.topicsArray[0].questions.forEach(question => {
      if (question.isChecked) {
        questionsChecked.push(question)
      }
    })

    if (questionsChecked.length == 0) {
      this.presentToast("You have not selected any questions. Please select a question deck to continue")
    }
           ...
           let navigationExtras: NavigationExtras = {
             state: {
               ...
               topic: this.topicsArray[0].title,
             },
           };
